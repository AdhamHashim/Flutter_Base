...
All rules in `.cursor/rules/` are auto-applied. Ensure you've internalized:
...
- NEVER: Positioned(left:/right:), Align(centerLeft/Right), EdgeInsets.only(left/right:), TextAlign.left, Directionality widget
...
Widget Reuse (إلزامي — أولوية عالية جداً):
...
- Fields: CustomTextFiled (label + validator + inputFormatters), AppDropdown<T>
...
- NEVER: Positioned(left/right), Directionality, TextAlign.left
...
- Text: const TextStyle().setMainTextColor.s14.medium — font reduced 1-2sp from Figma
...
8. Font sizes reduced 1-2sp from Figma | Screen body padding > 12px reduced 2-4px
...
18. Pre-delivery checklist from `flutter-feature-development.mdc` PHASE 7 — all items ✓
