...
> **لو الـ service فشلت، الـ dropdown بس هو اللي يتأثر — الشاشة باقيها تشتغل عادي.**

```dart
// ✅ CORRECT — isolated API call inside the dropdown widget itself
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      ...
      child: AsyncBlocBuilder<GetCitiesCubit, List<CityEntity>>(
        ...
        skeletonBuilder: (_) => AppDropdown<CityEntity>(
          items: const [], label: LocaleKeys.city.tr(), isLoading: true,
        ),
      ),
    );
  }
...
// ❌ FORBIDDEN — wrapping entire screen with BlocBuilder for a small dropdown
class _MyScreenBody extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return AsyncBlocBuilder<GetCitiesCubit, List<CityEntity>>(  // ← if this fails, ENTIRE screen stops!
      builder: (ctx, cities) => Column(children: [
        _HeaderWidget(),
        _FormWidget(),
        AppDropdown(items: cities, ...),  // ← only this needs the data
        _SubmitButton(),
      ]),
    );
  }
...
**Rules:**
- Dropdown / chip / small data widget بيجيب data من API → يكون عنده `BlocProvider` + `AsyncBlocBuilder` خاص بيه
- لو الـ service فشلت → الـ widget ده بس يعرض error، باقي الشاشة تشتغل عادي
- ده بيمنع إن failure في service واحدة صغيرة يوقف الشاشة كلها
